...
from parse_dat_to_csv import parse_cfg
...
def find_second_positive_mutation(rms_diffs, threshold, first_pos_end_idx):
    ...
    while i < n - recovery_length:
        ...
        if recovered:
            # 恢复期后，查找第二次正突变
            # find_first_positive_mutation 返回的是切片内局部索引，需叠加 search_from 偏移
            # 转换为 rms_diffs 中的绝对索引后返回，调用方无需再叠加偏移
            search_from = i + recovery_length
            val, local_idx = find_first_positive_mutation(rms_diffs[search_from:], threshold)
            if local_idx is not None:
                return val, search_from + local_idx
            return None, None
...
def find_second_negative_mutation(rms_diffs, threshold, second_pos_end_idx):
    ...
    # 返回绝对索引（已叠加 search_from 偏移），调用方无需再叠加
    val, local_idx = find_first_negative_mutation(rms_diffs[search_from:], threshold)
    if local_idx is not None:
        return val, search_from + local_idx
    return None, None
...
def extract_statistics_and_events(csv_path, output_dir=None):
    """
    从CSV文件提取RMS统计值和开关量事件

    参数:
        csv_path: CSV文件路径（由parse_dat_to_csv.py生成）
        ...
        if first_pos_diff is not None and first_neg_diff is not None:
            ...
            if search_start < len(rms_diffs):
                ...
                if sec_pos_val is not None and sec_pos_local_idx is not None:
                    ...
                    second_pos_diff_idx = sec_pos_local_idx  # 函数已返回绝对索引，无需再叠加偏移
                    # 记录第二次正突变时间和索引
                    ...
                    if sec_neg_val is not None and sec_neg_local_idx is not None:
                        ...
                        second_neg_diff_idx = sec_neg_local_idx  # 函数已返回绝对索引，无需再叠加偏移
    ...
    # 按通道分组计算持续时间
    durations_list = []
    for ch_name, group in true_events.groupby('通道名称', group_keys=False):
        durations_list.extend(calculate_duration(group).tolist())
    true_events['持续时间(ms)'] = durations_list
    ...
    for csv_file in args.csv_files:
        ...
        try:
            ...
            if output_dir is None:
                ...
                #  在技能安装目录下会错误地指向 .workbuddy/output）
                output_dir = str(Path.cwd() / 'output')

            extract_statistics_and_events(csv_path, output_dir=output_dir)
        except Exception as e:
            print(f"  错误: {e}")
            import traceback
            traceback.print_exc()
