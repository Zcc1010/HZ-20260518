...
export interface SettingCheckJob {
  id: string;
...
interface SettingCheckJobListProps {
  selectedJobId: string | null;
  onSelect: (job: SettingCheckJob) => void;
}

export function SettingCheckJobList({ selectedJobId, onSelect }: SettingCheckJobListProps) {
  ...
  const filteredJobs = jobs.filter((j) => {
    ...
    return (j.station ?? "").toLowerCase().includes(stationFilter.toLowerCase()) ||
           (j.device ?? "").toLowerCase().includes(stationFilter.toLowerCase());
  });
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {paginatedJobs.length === 0 ? (
          <div className="flex items-center justify-center py-8 text-sm text-[#888]">
            {stationFilter ? "没有匹配的任务" : "暂无任务"}
          </div>
        ) : (
          <div className="space-y-0.5 px-2">
            {paginatedJobs.map((job) => {
              const isSelected = job.id === selectedJobId;
              const displayName = job.station || job.device || "-";
              return (
                <div
                  key={job.id}
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#888]">{job.device || ""}</span>
