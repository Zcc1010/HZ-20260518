...
export function SetupGuideDialog() {
  ...
  return (
    <Dialog open={shouldShowGuide} onOpenChange={() => handleClose()}>
      <DialogContent 
        className="brand-panel border-[var(--brand-line)] sm:max-w-lg" 
        onPointerDownOutside={(e) => e.preventDefault()} 
        onEscapeKeyDown={(e) => e.preventDefault()}
      >
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#dff1ff] shadow-[0_10px_24px_rgba(89,135,255,0.16)]">
              {step === 0 && <AlertCircle className="h-5 w-5 text-[#4f7cff]" />}
              {step === 1 && <Key className="h-5 w-5 text-[#4f7cff]" />}
            </div>
            <DialogTitle>
        ...
        <div className="space-y-4 py-4">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-[var(--brand-line)] bg-[linear-gradient(135deg,rgba(213,238,255,0.92),rgba(255,255,255,0.98))] p-4">
                <p className="brand-display brand-gradient-text text-lg">{BRAND_NAME}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t("setupGuide.description")}</p>
              </div>
              <div className="grid gap-3">
                <div className="brand-hover-border flex items-center gap-3 rounded-2xl border border-[var(--brand-line)] bg-white/80 p-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#dff1ff]">
                    <span className="text-sm font-semibold text-[#4f7cff]">1</span>
                  </div>
              ...
              <div className="flex justify-end">
                <Button onClick={() => setStep(1)} className="gap-2 bg-gradient-to-r from-[#5e7fff] to-[#7b67ff] text-white shadow-[0_16px_32px_rgba(94,127,255,0.24)] hover:from-[#4d70ff] hover:to-[#6e58ff]">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between pt-2">
                ...
                <Button onClick={handleSaveProvider} disabled={updateProvider.isPending || updateAgent.isPending} className="gap-2 bg-gradient-to-r from-[#5e7fff] to-[#7b67ff] text-white shadow-[0_16px_32px_rgba(94,127,255,0.24)] hover:from-[#4d70ff] hover:to-[#6e58ff]">
