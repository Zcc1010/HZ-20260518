...
async function fetchJob(jobId: string): Promise<SettingCheckJob> {
  const res = await fetch(withBasePath(`/api/setting-check/jobs`));
  if (!res.ok) throw new Error("Failed to fetch jobs");
  const jobs: SettingCheckJob[] = await res.json();
  const job = jobs.find((j) => j.id === jobId);
  if (!job) throw new Error("未找到定值校核任务");
  return job;
}
...
export default function SettingCheckChatPage() {
  const { jobId } = useParams<{ jobId: string }>();
  const navigate = useNavigate();

  const [job, setJob] = useState<SettingCheckJob | null>(null);
      ...
      <div className="flex h-screen flex-col items-center justify-center bg-gradient-to-br from-[#f0f7fa] to-[#e8f4f3]">
        <div className="flex flex-col items-center gap-6 rounded-3xl bg-white/80 px-12 py-10 shadow-lg backdrop-blur-sm">
          ...
          <div className="text-center">
            <p className="text-lg font-medium text-[#333]">加载定值校核报告</p>
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-gradient-to-br from-[#f0f7fa] to-[#e8f4f3]">
        <div className="flex flex-col items-center gap-6 rounded-3xl bg-white/80 px-12 py-10 shadow-lg backdrop-blur-sm">
          ...
          <div className="text-center">
            <p className="text-lg font-medium text-[#333]">未找到报告</p>
            {jobId && (
              <p className="mt-1 text-sm text-[#555]">任务ID: {jobId}</p>
            )}
            <p className="mt-2 text-sm text-[#888]">{error || "该定值校核任务暂无报告"}</p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="flex w-[50%] shrink-0 flex-col border-r border-[#e0e0e0] bg-white">
        ...
        <div className="flex items-center gap-3 border-b border-[#e8f0f0] px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs uppercase tracking-[0.18em] text-[#888]">定值校核报告</p>
      ...
      <div className="flex flex-1 flex-col bg-white">
        ...
        <div className="flex items-center gap-3 border-b border-[#e8f0f0] px-5 py-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="brand-display text-lg text-[#000]">{BRAND_NAME} 定值分析助手</h3>
          </div>
        ...
        <div className="border-t border-[#e8f0f0] px-4 py-3">
          <div className="flex items-end gap-2">
            <Textarea
              ...
              placeholder={`询问关于 ${displayStation} ${displayDevice} 的定值校核分析...`}
              rows={1}
