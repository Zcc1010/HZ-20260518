...
import { ArrowLeft, FolderOpen, ChevronRight, Plus, Search, X, FolderTree, ListTree, Upload, FileText, PanelLeftClose, PanelLeftOpen, LineChart, Scissors, Radio } from 'lucide-react'
...
import { WaveRecordChatPanel } from '@/components/agentplayground/waverecord/WaveRecordChatPanel'
...
import { listWorkspaces, getFileTree, createWorkspace, uploadFiles } from '@/components/agentplayground/waverecord/wave-record-api'
import type { FileNode } from '@/components/agentplayground/waverecord/wave-record-api'
import type { OutlineItem } from '@/components/agentplayground/settingcheck/setting-check-parse'
import { useNavigate, useSearchParams } from 'react-router-dom'
import ChannelTree from '@/components/agentplayground/waverecord/ChannelTree'
import WaveformView from '@/components/agentplayground/waverecord/WaveformView'
import HdrView from '@/components/agentplayground/waverecord/HdrView'
import SubagentExtract from '@/components/agentplayground/waverecord/SubagentExtract'
import FileUploader from '@/components/agentplayground/waverecord/FileUploader'

type LeftTab = 'files' | 'outline' | 'channels'
type CenterTab = 'file' | 'waveform' | 'hdr' | 'extract'

export default function WaveRecordWorkspacePage() {
  ...
  const [leftWidth, setLeftWidth] = useState(() =>
    Number(localStorage.getItem('wrws-leftWidth')) || 260,
  )
  const [rightWidth, setRightWidth] = useState(() =>
    Number(localStorage.getItem('wrws-rightWidth')) || 560,
  )
  const [leftCollapsed, setLeftCollapsed] = useState(() =>
    localStorage.getItem('wrws-leftCollapsed') === 'true',
  )
  ...
  useEffect(() => {
    localStorage.setItem('wrws-leftWidth', String(leftWidth))
    localStorage.setItem('wrws-rightWidth', String(rightWidth))
  }, [leftWidth, rightWidth])

  useEffect(() => {
    localStorage.setItem('wrws-leftCollapsed', String(leftCollapsed))
  }, [leftCollapsed])
  ...
  useEffect(() => {
    if (workspacePath) {
      ...
      const es = new EventSource(`/api/wave-record-workspace/workspaces/${encodeURIComponent(ws)}/events`)
      es.onmessage = (e) => {
        try {
  ...
  useEffect(() => {
    if (jobId) {
      setLoading(true)
      fetch(`/api/wave-record-parser/jobs/${jobId}`)
        .then(res => res.json())
        .then(job => {
          if (job && job.id) {
            ...
            listWorkspaces().then(workspaces => {
              if (workspaces.includes(wsName)) {
                ...
                fetch(`/api/wave-record-parser/jobs/${jobId}/copy-to-workspace`, {
                  method: 'POST',
  ...
  useEffect(() => {
    ...
    if (ext === 'hdr') {
      setCenterTab('hdr')
    } else if (ext === 'cfg' || ext === 'dat' || ext === 'rms.csv' || ext === 'events.csv') {
      ...
      <header className="h-12 bg-white border-b border-gray-200 flex items-center px-4 shrink-0 gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-md bg-blue-600 flex items-center justify-center">
            <span className="text-white text-xs font-bold">波</span>
          </div>
          <h1 className="text-sm font-semibold text-gray-800 whitespace-nowrap">录波解析</h1>
        </div>
        <div className="w-px h-5 bg-gray-200" />
        <Button
          variant="ghost"
      ...
      <div className="flex-1 flex overflow-hidden">
        {leftCollapsed ? (
          <div className="shrink-0 bg-white border-r border-gray-200 flex flex-col items-center py-2 gap-2">
            ...
            <button
              onClick={() => { setLeftCollapsed(false); setLeftTab('channels') }}
              className={`p-1.5 rounded-md transition-colors ${leftTab === 'channels' ? 'bg-teal-50 text-teal-600' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}`}
              title="通道"
            >
                ...
                {leftTab === 'files' ? (
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="p-2 border-t border-gray-200">
                      <FileUploader onFilesLoaded={() => setCenterTab('waveform')} />
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="flex-1 overflow-hidden bg-gray-50 flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {centerTab === 'waveform' && <WaveformView />}
            {centerTab === 'hdr' && <HdrView />}
            {centerTab === 'extract' && <SubagentExtract />}
          </div>
        </div>
        ...
        <div style={{ width: rightWidth }} className="shrink-0 bg-white border-l border-gray-200 overflow-hidden">
          <WaveRecordChatPanel workspacePath={workspacePath} />
        </div>
      </div>
