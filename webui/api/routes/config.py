...
from webui.utils.log_files import get_log_base_path, read_recent_log_lines
...
def get_logs(
    ...
    log_base = get_log_base_path(get_config_path().parent)
    try:
        content, log_file = read_recent_log_lines(log_base, lines=lines, keyword=keyword)
        if not content and not log_file.exists():
            return {"content": "Log file not found.", "path": str(log_file)}
        return {"content": content, "path": str(log_file)}
    except Exception as e:
        return {"content": f"Error reading logs: {e}", "path": str(log_base)}
...
async def upload_to_s3(
    ...
    if len(data) > _MAX_UPLOAD_BYTES:
        raise HTTPException(
            ...
            f"Upload exceeds {_MAX_UPLOAD_LABEL} limit",
        )
