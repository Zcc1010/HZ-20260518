...
def _workspace_root() -> Path:
    """~/.nanobot/agentplayground/wave-record-parser/workspace/"""
    home = Path.home()
    root = home / ".nanobot" / "agentplayground" / "wave-record-parser" / "workspace"
...
async def create_workspace(request: Request):
    ...
    # 创建默认目录结构
    (target / "录波源文件" / "保护录波").mkdir(parents=True, exist_ok=True)
    (target / "录波源文件" / "故障录波").mkdir(parents=True, exist_ok=True)
    (target / "报告").mkdir(parents=True, exist_ok=True)
    return {"name": name}
