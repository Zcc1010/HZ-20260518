...
import typer
from webui.utils.log_files import get_log_base_path, latest_log_path, read_recent_log_lines
...
def _log_base_file() -> Path:
    return get_log_base_path(Path.home() / ".nanobot")


def _log_file() -> Path:
    return latest_log_path(_log_base_file())
...
def webui_start(
    ...
    daemon: bool = typer.Option(
        ...
        help="Run in the background and return immediately. "
             "PID written to ~/.nanobot/webui.pid, logs use ~/.nanobot/webui.log as the base path.",
    ),
