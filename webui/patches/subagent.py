...
async def _save_sub_tool_to_session(
    ...
    try:
        ...
        summary = f"[子智能体已完成] 工具：{', '.join(tools_called[-6:])}。"
        if final_text:
...
def apply() -> None:
    ...
    async def _announce_result_patched(
        ...
        if channel == "web":
            ...
            content = (
                f"[子任务 '{label}' {status_text}]\n\n任务：{task}\n\n"
                f"结果：\n{result}\n\n"
                "请把结果自然地整理给用户，保持简洁（1-2 句）。"
                "不要提及“subagent”或任务 ID 之类的技术细节。"
            )
        ...
        async def _process_message_patched(self, msg, session_key=None, on_progress=None, **kwargs):
            ...
            if msg.channel == "system" and msg.sender_id == "subagent" and subagent_label:
                ...
                try:
                    ...
                    announce_prefix = f"[子智能体“{subagent_label}”"
